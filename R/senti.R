...
#' @importFrom igraph graph.data.frame E V shortest_paths
#' @export
...
senti <- function(edgelist, target, positiveLexicon=system.file("extdata", "SeedsPos.txt", package = "senet"), negativeLexicon=system.file("extdata", "SeedsNeg.txt", package = "senet")) {
    ...
    for (negativeWord in negativeWords) {
        ...
        result <- calculateDistance(graph, targetV, negV, positiveWord)
        targetToNegMat_weight <- result[1]
        targetToNegMat_length <- result[2]

        # reverse
        result <- calculateDistance(graph, negV, targetV, positiveWord)
...
calculateDistance <- function(graph, from, to, exclude) {
    path <- igraph::shortest_paths(graph, from, to=to, mode="out", output="both")
    pure <- TRUE
    for (vertexIndex in path$vpath[[1]]) {
    ...
    d <- 0
    if (pure) {
        for (edgeIndex in path$epath[[1]]) {
            weight <- E(graph)$weight[edgeIndex]
            d <- d + weight
        }
    }
    return(c(d, length(path$epath[[1]])))
}
